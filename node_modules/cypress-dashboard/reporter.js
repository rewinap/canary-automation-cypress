...
function MyReporter(runner, options) {
    ...
    const reportType = options.reporterOptions.reportType && ["dashboard", "details"].includes(options.reporterOptions.reportType) ? options.reporterOptions.reportType : "dashboard";
    ...
    const getTestNamespace = test => {
        ...
        while (parent) {
            if (parent.title === "" && !!parent.file) testNamespace.push(parent.file.replace("cypress\\integration\\", "").replace("cypress/integration/", ""));
        ...
        if (API_KEY) {
            ...
            if (isLocalDeployment) {
                fs.writeFileSync(`${reportDir}/index.html`, indexFile.replace(/URL_PLACEHOLDER\s*/gi, `${directory}/${reportDir}`)
                    .replace(/<script type="text\/javascript"><\/script>/gi, `<script type='text/javascript'>window.config=${JSON.stringify(config)};window.results=${JSON.stringify(results)};window.reportType='${reportType}';</script>`))
            } else {
